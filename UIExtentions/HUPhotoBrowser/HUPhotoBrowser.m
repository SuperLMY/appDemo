    ...
    if (self.URLStrings) {
        ...
        if (indexPath.row != _index) {
            if (_placeholderImage) {
                [cell stopAnimating];
                [cell.imageView sd_setImageWithURL:url placeholderImage:_placeholderImage completed:^(UIImage * _Nullable image, NSError * _Nullable error, SDImageCacheType cacheType, NSURL * _Nullable imageURL) {
        ...
        if ([obj isKindOfClass:[NSString class]]) {
            ...
            if (indexPath.row != _index) {
                ...
                UIImage *placeHolder = _tmpImageView.image;
                if ([LocalManager checkUrl:obj record:[LocalManager shareLocalRecord].keepRecordId]) {
                    [cell.imageView readImageInLibary:[LocalManager shareLocalRecord].keepRecordId UrlString:obj];
                }else{
                    [cell.imageView sd_setImageWithURL:url placeholderImage:placeHolder completed:^(UIImage * _Nullable image, NSError * _Nullable error, SDImageCacheType cacheType, NSURL * _Nullable imageURL) {
                        __strong __typeof(wself) strongSelf = wself;
                        __strong __typeof(weakCell) strongCell = weakCell;
...
- (void)imageSavedToPhotosAlbum:(UIImage *)image didFinishSavingWithError:(NSError *)error contextInfo:(void *)contextInfo {
    ...
    if(error != nil){
        msg = @"保存图片失败";
        [JGProgressHUD showErrorHUD:msg];
    }
    else{
        msg = @"保存图片成功";
        [JGProgressHUD showSuccessHUD:msg];
    }
